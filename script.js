...
async function restoreSession() {
  try {
    const savedSession = sessionStorage.getItem('sb-session') || localStorage.getItem('sb-session');
    if (savedSession) {
      const parsed = JSON.parse(savedSession);
      const { data, error } = await client.auth.setSession(parsed);
      if (error) console.warn("Fehler beim Wiederherstellen der Session:", error.message);
      else console.log("Session erfolgreich wiederhergestellt");
    } else {
      console.log("Keine gespeicherte Session gefunden");
    }
  } catch (err) {
    console.error("Fehler in restoreSession():", err);
  }
}
...
async function checkSession() {
  try {
    ...
    if (error || !data?.session) {
      console.log("Keine aktive Session gefunden oder Fehler:", error);
      document.querySelectorAll('article[class^="lz"]').forEach(a => {
        a.innerHTML = `
          <div class="geschuetzt">
            <img src="/b/schloss.png" alt="Bild eines Schlosses">
            <p>Um diesen Beitrag sichtigen zu können, ist ein erweiterter Lesezugriff notwendig. Logge dich ein um zu überprüfen, ob du über diesen verfügst.</p>
            <p><u><a href="/t/auth.html?redirect=${encodeURIComponent(window.location.href)}">Zum Login</a></u></p>
          </div>`;
  ...
  try {
    ...
    for (const article of articles) {
      ...
      if (hasAccess === true || hasAccess === 1) {
        ...
        article.innerHTML = `
          <div class="geschuetzt">
            <img src="/b/schloss-neg.png" alt="Bild eines Schlosses">
            <p>Dieser Beitrag erfordert einen erweiterten Lesezugriff. Der Account, mit dem du eingeloggt bist, verfügt über diesen momentan nicht. Das heißt dass du diesen Text nicht einsehen kannst.</p>
          </div>`;
        if (komments) komments.style.display = 'none';
      }
    }
...
window.addEventListener('storage', (e) => {
  if (e.key === 'sb-session' && !e.newValue) {
    console.log("Logout erkannt — Session löschen");
    client.auth.signOut();
  }
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initFullscreenToggle(config) {
        ...
        if (!icon.hasAttribute("data-original")) {
            icon.setAttribute("data-original", config.maximizeIcon);
        }

        function updateIcon(isFullscreen) {
            const baseSrc = isFullscreen ? config.minimizeIcon : config.maximizeIcon;
            icon.setAttribute("data-original", baseSrc);

            const isDark = document.body.classList.contains('dark-mode');
            icon.src = isDark ? makeDarkFromLight(baseSrc) : baseSrc;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function updateUI() {
    ...
    if (user) {
      ...
      resultDiv.innerText = `Eingeloggt als ${user.email.split("@")[0]} ✅`;
    } else {
  ...
  async function loadComments() {
    ...
    for (const c of comments) {
      ...
      if (c.user_id) {
        ...
        nameSpan.textContent = userData?.b_name ? `${userData.b_name}, ` : "Unbekannt, ";
      } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  externLinks.forEach(link => {
    ...
    icon.src = isDarkMode ? "/b/dark/extern-dark.png" : "/b/extern.png";
    icon.alt = "";
